...
type ollamaService struct {
	ollamaConfig config.OllamaModelConfig
}

func NewOllamaService(ollamaConfig config.OllamaModelConfig) *ollamaService {
...
func (o *ollamaService) StreamChat(requestBody RequestBody) (<-chan ResponseBody, error) {
	...
	go func() {
		...
		requestBody.Messages = append([]Message{{Role: "system", Content: o.ollamaConfig.SystemPrompt}}, requestBody.Messages...)
		...
		resp, err := http.Post(o.ollamaConfig.BaseURL+"/api/chat", "application/json", bytes.NewBuffer(jsonBody))
		if err != nil {
